<?php
	...
	Class fieldAdvancedTaglist extends Field {
		public function __construct(&$parent){
			parent::__construct($parent);
			$this->_name = 'Advanced Tag List';
		}
		...
		function findAllTags(){			
			...
			foreach($this->get('pre_populate_source') as $item){
				$result = $this->_engine->Database->fetch(sprintf($sql, ($item == 'existing' ? $this->get('id') : $item)));

				if(!is_array($result) || empty($result)) continue;
				
				foreach($result as $r){
					$values[$r['value']] += $r['count'];
				}
